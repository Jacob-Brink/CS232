...
public class Client {
	 ...
	 * @params: none
	 * 
	 * @description: sends user's messages to ceasar cipher server and returns
	 * server's response
	 */
	public static void prompt() {

		// Here we open resources in the try with statement so all resources opened are
		// closed correctly
		try (Socket sock = new Socket(host, port);
			...
			while (true) {
				...
				if (rotationStepComplete) {
					...
					// if response is valid, proceed to ceasar cipher text sending and
					// receiving (sometimes the example server gave back incorrect numbers)
